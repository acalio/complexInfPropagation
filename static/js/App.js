...
const stateScale = d3.scaleOrdinal()
    .domain(["inactive", "quiescent", "active", "comp"])
    .range(["#75739F", "#41A368", "#FE9922", "#FE0029"])
...
class Trends extends React.Component {
    constructor(props) {
        ...
        this.margins = {x:40, y: 30}
        this.labelMap = {"active": "A.", "quiescent": "Q.", "comp": "A. (comp)"}
    }
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        this.seriesHash.forEach((v,k,m,i) => {
            ...
            //console.log(points)
            var line_ = d3.line()
                .x( d=> this.margins.x+ this.xScale(d.x))
                .y(d=>this.margins.y + this.yScale(d.y))
    ...
    render()
    {
        ...
        {
            var circles = v.map( (p,i) =>
                <circle r={5}
                        cx={this.margins.x + this.xScale(i)}
                        cy={this.margins.y + this.yScale(p)}
                        fill={stateScale(k)}
                />
            )
...
class App extends React.Component {
    ...
    toggleActive(i)
    {
        ...
        this.state.points.push({
           name: "active", x:0, y:this.state.active
        })

        if(this.state.model>0)
            this.state.points.push({
                name:"comp", x:0, y:this.state.activeComp
            })
            ...
            json.forEach((nList, i) => {
                if(nList.length===0) //end of the simulation
                    ...
                    setTimeout(() => {
                        ...
                        this.state.points.push({
                            name: 'active', y: this.state.active, x: round + 1
                        })
                        this.state.points.push({
                            name: 'quiescent', y: this.state.quiescent, x: round + 1
                        })

                        if(this.state.model > 0)
                            this.state.points.push({name: 'comp', y: this.state.activeComp, x: round+1})
            ...
            <div>
                ...
                <div className={"row"}>
                        ...
                        <div className={"card card-small"}>
                            <div className={"card-header border-bottom"}>
                                ...
                                <form className={"form-inline"}>
                                    ...
                                    <button type={"button"} onClick={this.updateNetwork} className={"btn mb-2"} disabled={this.state.running}
                                            value={"Create Graph"}>
                                        Create Graph
                                    </button>
                                    <select onChange={this.modelSelection} className={"form-control custom-select form-control sm-custom-select-sm"} value={this.state.model} >
                                        <option value={0}> Non-Competitive </option>
                                        <option value={1}> Semi-Progressive </option>
                                        <option value={2}> Non-Progressive </option>
                                    </select>
                                    <button type={"button"} onClick={this.run} className={"btn btn-accent"}
                                            value={"Run"} disabled={this.state.running}>
                                        Run
                                    </button>
                    ...
                    <StatsBlock statTitle={"Active Users"} value={this.state.active}
                                percentage={this.state.N === 0 ? 0 : (this.state.active / this.state.N) * 100}/>
                    {this.state.model > 0 &&
                        <StatsBlock statTitle={"Active Users (comp)"} value={this.state.activeComp}
                                percentage={this.state.N==0 ? 0 : (this.state.activeComp / this.state.N) * 100}/>
                    }
